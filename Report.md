...
KMeans Clustering model from Scikit-learn is then trained on this onehot encoding dataframe (with number of clusters = 6). It is an unsupervised learnning algorithm for classification based on the closeness of vectors in Euclidean space. No of clusters was chosen to be 6 so that it is large enough to offer specific options to the user, and at the same time ensuring that no single element clusters are formed.<br>
The clusters are marked on the map in different colour.<br>
